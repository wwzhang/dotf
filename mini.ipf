...
Function ave_traces()
	...
	CurveFit/M=2/W=0 exp, NAve[100,450]/X='Time-0000 (s)'[100,450]/D
	wave W_coef
	variable decay
	decay = 1/W_coef[2] * 1000
	print "Decay =", decay
END
...
Function PSC_quik_figs()
	String group_1="folder1", group_2="folder2"
...
//	TextBox/B=1/A=LB/F=0 "$group_1"; TextBox/B=1/A=RB/F=0 "$group_2"; Label left "Instantaneous Frequency (Hz)"; Style_group()
...
Function PSC_cumu_plot()
	setdatafolder root:
	String group_1="folder1", group_2="folder2"
	Prompt group_1, "Group 1 Name";
	Prompt group_2, "Group 2 Name";

	DoPrompt "Generate figures", group_1, group_2
	    
		if (V_Flag)
			return -1	// User canceled 
		endif
	//wave root:$(group_1):w_per_amp, group_1:w_per_isi, group_2:w_per_amp, group_2:w_per_isi	
	
	display  /N=cumu_amp root:$(group_1):w_per_amp;
	appendtograph root:$(group_2):w_per_amp; style_cumu_amp(); Legend/C/N=text0/J/F=0;
	display /N=cumu_isi root:$(group_1):w_per_isi;
	appendtograph root:$(group_2):w_per_isi; style_cumu_isi(); Legend/C/N=text0/J/F=0;
END

Menu "Miniature"
	"1. Merge Experiments", MergeSelectedtoCurrent()
	"2. Process raw datas after merge", PSC_raw_datas()
	"3. Display Parameter figs", PSC_quik_figs()
	"4. cumulative analysis for one folder", PSC_cumu()
	"5. CUmulative plots", PSC_cumu_plot()
...
//shortcut for data browser
Menu "Macros"
	"Open Data Browser/1", /Q, Execute "CreateBrowser"	// Cmd-1
//	"Open Data Browser/F5", /Q, Execute "CreateBrowser"	// F5
End
