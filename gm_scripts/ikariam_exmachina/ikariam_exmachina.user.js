...
// @history		0.61 Reduced initial load time when searching for player cities in world map overview
// @history		0.61 Added loading animating during player search to world map overview
// @history		0.61 Fixed bug when switching cities via traditional city selector on send troops to pillage
// @history		0.60 Fixed broken indication of player islands when searching 
// @history		0.59 Made the world map a little lighter on memory
// @history		0.59 Improved population density "heatmap" in world overview (full islands are now black)
// @history		0.59 Added icons to represent message types when using message preview (currently only works in English)
// @history		0.58 Ikariam Army Helper script included with permission from author
// @history		0.57 Fixed broken finances overview in world overview
// @history		0.56 Changed hover indicator in world map overview
// @history		0.56 Fixed island view breaking when closing world map overview
// @history		0.56 Fixed multiple issues with searching for several players without reloading the page
// @history		0.56 Fixed no player search results showing single result
// @history		0.56 Added search icon next to player names in island view
// @history		0.56 Added search icon next to player name in city occupation warning in city view
// @history		0.56 Added search icon next to player names in inbox
// @history		0.56 Added search icon next to player names in alliance views
// @history		0.56 Added search icon next to player names in highscore
// @history		0.55 Hid archive tab when viewing combat report details with Anti-Plus enabled 
// @history		0.55 Attempted to fix calculation of capacity for high level warehouses (capacity ending in "k" to represent thousands)
// @history		0.55 Added time to upgrade to building resource display on mouseover in city view
// @history		0.55 Added time to upgrade building to tooltips in buildings overview
// @history		0.54 Fixed Advisor notifications not showing
// @history		0.53 Fixed extra column bug in empire overview created in v0.52
// @history		0.52 Building required resources now auto updates in city view
// @history		0.52 Fixed potential bug in building position that might cause script to fail when drawing building lists
// @history		0.52 Fixed potential bug in auto build that might cause script to fail in certain circumstances
// @history		0.51 Searching for a player name in world map overview now marks islands
// @history		0.50 Added search by player name to world map overview (island marking coming soon);
// @history		0.49 Added player city list to island page
// @history		0.48 Added tradegood type to spy targets overview under research advisor
// @history		0.48 Fixed some minor glitches having to do with targets
// @history		0.47 Added mouseover tooltip of resources missing/required to buildings overview under town advisor
// @history		0.47 Added resourde details
// @history		0.46 Removed banner add from game
// @history		0.45 Added message previews
// @history		0.45 Changed timeout for auto-military overview reload on mouseover to 5 10 minutes instead of 1 minute
// @history		0.45 Blocked "move city" feature in anti-plus on colonize view
// @history		0.44 Building levels in city now show resources that will be spent for buildable buildings
// @history		0.43 Added current construction indication to buildings overview under town advisor
// @history		0.43 Added current construction indication to building lists
// @history		0.43 Fixed links in buildings overview for Google Chrome version
// @history		0.42 Fixed positioning of world map overview in Google Chrome version
// @history		0.41 Added world map overview button
// @history		0.40 Small aesthetic change to incomming troop numbers in military overview
// @history		0.40 Fixed some poor coding choices related to reading military information
// @history		0.39 Added ships to military overview
// @history		0.39 Fixed display glitches on ship description in help
// @history		0.39 Fixed fatal error when using resource links and viewing occupied cities
// @history		0.39 Fixed URLs in military overview for Google Chrome version
// @history		0.39 Added barracks level to barracks tooltips in military overview
// @history		0.39 Added link to shipyards in military overview
// @history		0.39 Added city levels to city names in military overview
//
// ==/UserScript==
...
ScriptUpdater.check(80545, "0.95");


var isChrome = navigator.userAgent.match(/Chrome/i);

Config.prefix = document.domain;
Config.footerHtml = '<div>Help support this script\'s continued development &nbsp; &nbsp; ♥ <a target="_blank" href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=phasmaexmachina%40live%2ecom&item_name=Donation%20to%20support%20Ikariam%20ExMachina&item_number=userscripts&currency_code=USD&bn=PP%2dDonationsBF%3abtn_donate_SM%2egif%3aNonHosted" title="Buy me a cup of coffee :-)"><img src="https://www.paypal.com/en_US/i/btn/btn_donate_SM.gif" style="vertical-align:middle;" title="Suggested donatioation $5-$10 but any and all amounts are appreciated"></a> ♥</div>';
...
Config.settings = {
	...
	"Events":{
		...
		fields:{
			...
			eventsEmailHtml:{
				type:'html',
				value:'  <p style="border-top:1px solid #ccc; padding-top:1em !important;"><strong>Email Settings</strong> - see "Email Settings" on <a href="http://userscripts.org/scripts/show/80545" target="_blank">this page</a> for more information.</p>',
			},
			emailNoticeUrl:{
				type:'text',
				label:'Server',
...
ExMachina = {
	...
	expiredMovements:{},
	init:function() {
		if(document.location.toString().match(/s\d+\..*?ikariam\./) && document.body.id != 'errorLoggedOut' && $('#mainview').size() == 1) {	// in game
			IkaTools.init();
			...
			function antiPlus() { 
				...
				if(parseInt(mainVersion) > 3)
					GM_addStyle('.premiumExchange, .premium, .ambrosia, .plusteaser { display:none !important; }\
								#globalResources li.transporters, #globalResources li.gold { position:relative; top:-28px; }');
				else
					GM_addStyle('.premiumExchange, .premium, .ambrosia, .plusteaser { display:none !important; }\
								#globalResources li.gold { position:relative; top:-28px; }');
			}
			...
			function resourceDetails() {
				...
				$('#cityResources ul.resources li').each(function(i) {
					if (i > 1) {
						...
						ttHtml += '<tr><td>Capacity:  </td><td>' +  IkaTools.addCommas(capacity) + ' (' + capacityPercent + '%)</td</tr>';
						// calculate qty safe
						var numWarehouses = 0;
						...
						if(onHand > 0)
							ttHtml += '<tr><td>Safe:   </td><td> ' + IkaTools.addCommas(safe) + ' (' + safePercent + '%)</td></tr>';
						if (unsafe > 0) {
							var lootableColor = unsafePercent > 50 ? ' style="color:#a00; font-weight:bold"' : '';
							ttHtml += '<tr><td' + lootableColor + '>Lootable:   </td><td' + lootableColor + '> ' + IkaTools.addCommas(unsafe) + ' (' + unsafePercent + '%)</td></tr>';
						}
						if (change != 0) {
							// display income
							ttHtml += '<tr><td style="padding-top:.75em !important;">Income:  </td><td style="padding-top:.75em !important;">' + (change > 0 ? '+' : '') + change + ' (' + (change > 0 ? '+' : '') + IkaTools.addCommas(change * 24) + ' / day)</td></tr>';
							...
							ttHtml += '<tr><td' + fullColor + '>' + (change > 0 ? 'Full' : 'Empty') + ':   </td><td' + fullColor + '>' + (days > 0 ? days + 'd ' : '') + (hours > 0 ? hours + 'h ' : '') + (minutes > 0 ? minutes + 'm ' : '') + '</td></tr>';
						}
	...
	updateMilitaryOverview:function() {
		...
		for(var x = 0; x < IkaTools.shipTypes.length; x++) {
				if(typeof(totalShips[IkaTools.shipTypes[x]]) != 'undefined' && totalShips[IkaTools.shipTypes[x]] > 0)
					html += '<th class="ship_' + IkaTools.shipTypes[x] + '"> </th>';	
		}	
	...
	drawBuildingOverview:function() {
		...
		if(!IkaTools.buildingsOverview) {
					...
					for(var x = 0; x < IkaTools.buildingTypes.length; x++) {
						if(ownedBuildings[IkaTools.buildingTypes[x]]) {
							...
							if(building) {
								...
								if(isUnderConstruction)
									html.push('<span style="padding:0 1px;">»</span>' + (parseInt(building.level) + 1));
								html.push('</a>');
							} else
								html.push('-');							
							html.push('</td>');
			...
			// resource tooltip
			$('body').append('<div id="buildingOverviewTooltip" style="text-align:left; z-index:40000; background:#FFFBDB; padding:5px 8px; font-size:11px; border:1px solid #542C0F; -webkit-border-radius: 5px; -moz-border-radius: 5px; position:absolute; top:0; left:0; display:none;">h </div>');
		...
		if(city.buildings) {
			...
			for(var i = 0; i < buildings.length; i++) {
				if (buildings[i].position) {
					...
					' (' +
					buildings[i].level +
					((construction && construction.position.toString() == buildings[i].position.toString()) ? '<span style="padding:0 2px;">»</span>' + (parseInt(buildings[i].level) + 1) : '') +
					')    ';
					var resourceNames = ["wood", "wine", "marble", "glass", "sulfur"];
	...
	drawMovements:function(movements) {
		...
		for(var i = 0; i < movements.length; i++) {				
			...
			movementsHtml += '<tr style="' + (i != movements.length - 1 ? 'border-bottom: 1px dotted rgb(157, 131, 106);' : '') + '">'+
				...
				'<td>' + (movements[i].direction == 'left' ? '<img src="http://' + document.domain + '/skin/interface/arrow_left_' + arrowColor + '.gif" style="height:10px;"/>' : ' ') + '</td>' +
				...
				'<td>' + (movements[i].direction == 'right' ? '<img src="http://' + document.domain + '/skin/interface/arrow_right_' + arrowColor + '.gif" style="height:10px;"/>' : ' ') + '</td>' +
				...
				'<td>' + ((movements[i].direction != 'left' && typeof(movements[i].abortHref) != 'undefined')? '<a href="http://' + document.domain + movements[i].abortHref + '" title="Abort Mission" style="padding:none; height:auto; width:auto;"><img src="' + ( '/skin/interface/btn_abort.gif') + '"/></a>' : ' ') + '</td>' +
				'</tr>';
		}
	...
	function handleCityInfoLoad(id) {
		...
		if(Config.get('playerCitiesInIslandView')) {
			if ($('#playerCityListWrapper').size() == 0) {
				$('#information .cityinfo').append('<li id="playerCityListWrapper" style="margin:1em 10px;"> </li>');
			...
			function drawScore(label, type) {
				if($('#exMachinaInlineScore').size() == 0)
					$('#information ul.cityinfo li.ally').after('<li id="exMachinaInlineScore" style="height:.5em; overflow:hidden;">&nbsp;</li>');
...
ExMachina.views.login = function() {
	if($('select#logServer').size() == 0) {
		if (document.domain == 'en.ikariam.com') $('#universe').append('<option value="s666.en.ikariam.com">Test Server</option>')
		...
		if(document.domain == 'en.ikariam.com') 
			$('#universe').append('<option value="s666.en.ikariam.com">Test Server</option>')
		GM_addStyle('#phasmaAutoLoginConfigButton:hover { opacity:1 !important; }');
		var settingsButton = $('<img id="phasmaAutoLoginConfigButton" src="' + ExMachina.icons.disk + '" style="float:right; margin-top:.5em; margin-left:1.2em; cursor:pointer; "/>');
		$('table#logindata').css('width', '100%');
				...
				$('#mainview .contentBox01h .content ul li', root).each(function() {													 
					...
					if(research = Librarian.getResearchByName(name)) {
						...
						$('a', this)[0].innerHTML = ('\
							...
							<div style="width: 70px; float:left; margin-right:2em;">' + (this.className == 'explored' ? '<span style="font-size:0;"> </span>' : research.completeIn)+ '</div>\
	...
	init:function() {
		...
		if($('#buildingUpgrade').size() == 1 && upgradeBox && !IkaTools.buildingIsMaxed(building)) {		
			...
			if (building) {
				...
				var totalsHtml = '<table cellpadding="0" border="0" cellspacing="0" align="center" style="width:auto !important;"><tr><td style="padding:.25em 0 0 0; text-align:right;">' + IkaTools.addCommas(totalRequired) + '</td><td style="text-align:left;">&nbsp; total for level ' + (parseInt(building.level) + 1) + '</td><tr>'; 
				if (totalMissing > 0) {
					totalsHtml += '<tr><td style="padding:.25em 0 0 0; text-align:right;">' + IkaTools.addCommas(totalMissing) + '</td><td style="text-align:left;">&nbsp; more for 1:1 trade</td><tr>';
				...
				$('#buildingUpgrade ul.resources li').each(function() {
					var iconSrc = $(this).css('background-image').replace(/^url\("/, '').replace(/"\)/, '');
					$('.textLabel', this).html('');
					...
					resourcesHtml += '<td style="text-align:left;"> &nbsp; ' + $(this).text() + '</td><td style="text-align:left;">';
					if(cname != 'time') {
...
var IkaSearch = {
	init:function() {
		...
		if(document.location.toString().match(/ika-world/)) {
			...
			$('#breadcrumbs').prepend('<div style="width:1px; float:right; margin-right:23px;"> </div>');
		}
		if(Config.get('debugMode')) {
			IkaTools.config.debugMode = true;
...
ExternalScripts.ArmyHelper = function () {
	const languages = {
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		"arabic": {
			garrison: "القوات العسكرية",
			advisor: "المستشار",
			cityLevel: "مستوى دار البلدية",
			support: "مساعدة",
			artillery: "المدافع",
			air: "طيران",
			line1: "الخط الأمامي",
			line2: "معركة طويلة المدى",
			flankLeft: "الجناح الأيسر",
			flankRight: "الجناح الأيمن",
			submarine: "غواصة",
			reserve: "احتياطي",
			phalanx: "كتيبة",
			steamgiant: "عملاق بخاري",
			spearman: "حامل الرمح",
			swordsman: "مبارز",
			slinger: "مقلاع حجارة",
			archer: "رامي سهام",
			...
			catapult: "منجنيق",
			mortar: "هاون",
			gyrocopter: "طائرة مروحية",
			bombardier: "قاصف",
			cook: "طباخ",
			medic: "طبيب",
			ship_ram: "سفينة مزودة بقوة دفع",
			ship_flamethrower: "قاذف اللهب",
			ship_steamboat: "قوة دفع الطارة",
			ship_ballista: "سفينة حاملة لسلاح المرجام",
			ship_catapult: "سفينة مزودة بمنجنيق",
			ship_mortar: "سفينة هاون",
		...
		"french": {
			...
			line2: "Ligne de bataille de longue portée",
		...
		"greek": {
			...
			support: "Μονάδες Υποστήριξης",
			artillery: "Μονάδες Πυροβολικού",
			air: "Εναέριες Μονάδες",
			line1: "Κύρια Γραμμή Μάχης",
			line2: "Γραμμή Μάχης Μεγάλης Εμβέλειας",
			flankLeft: "Αριστερή Πλευρά",
			flankRight: "Δεξιά Πλευρά",
			submarine: "Υποβρύχιες Μονάδες",
			reserve: "Απόθεμα",
			phalanx: "Οπλίτης",
			...
			cook: "Μάγειρας",
			medic: "Γιατρός",
			ship_ram: "Σκάφος(-η) Έμβολο",
			ship_flamethrower: "Φλογοβόλο(-α)",
			ship_steamboat: "Σκάφος(-η) Κουπί-Ρόδα-Κριός",
			ship_ballista: "Σκάφος(-η) Βαλλιστών",
			ship_catapult: "Σκάφος(-η) Καταπελτών",
			ship_mortar: "Σκάφος(-η) Κονιάματος",
		...
		"hungarian": {
			garrison: "Helyőrség",
			advisor: "Tanácsadó",
			cityLevel: "Város méret",
			support: "Támogatás",
			artillery: "Tüzérségi",
			air: "Légi",
			line1: "Frontvonalban",
			line2: "Lövész vonalban",
			flankLeft: "Bal szárnyon",
			flankRight: "Jobb szárnyon",
			submarine: "Búvárhajó",
			reserve: "Foglalt",
			phalanx: "Hoplita",
			steamgiant: "Gőzóriás",
			...
			ship_steamboat: "Evezőkerék hajó",
			ship_ballista: "Balliszta",
			ship_catapult: "Katapult hajó",
			ship_mortar: "Ágyúhajó",
		...
		"italian": {
			...
			addSlot: "Aggiungi unità ad uno slot",
			removeSlot: "Rimuovi unità ad uno slot",
			fillRow: "Riempi una linea con le unità",
		...
		"latvian": {
			garrison: "Garnizons",
			advisor: "Konsultants",
			cityLevel: "Pilsētas līmenis",
			support: "Palīdzība",
			artillery: "Artilērija",
			air: "Gaisa spēki",
			line1: "Priekšējā līnija",
			line2: "Tālās kaujas līnija",
			...
			cook: "Pavārs",
			medic: "Ārsts",
			ship_ram: "Tarāna kuģis",
			ship_flamethrower: "Ugunsmetējs",
			ship_steamboat: "Dzenrata kuģis",
			ship_ballista: "Ballistiskais kuģis",
			ship_catapult: "Katapultas kuģis",
			ship_mortar: "Mīnmetēja kuģis",
			ship_submarine: "Zemūdene",
			addSlot: "Pievienot vienības uz vienu lauciņu",
			removeSlot: "Noņemt vienības no viena lauciņa",
			fillRow: "Aizpildīt vienu kaujas rindu ar vienībām",
			selectUnits: "Izvēlies vienības",
			assignNone: "Nevienu",
			assignAll: "Sūtīt visu",
			assignField: "Aizpildīt kaujas lauku",
			presetsTitle: "Iepriekšplānots sastāvs",
			presetsAdd: "Pievienot jaunu sastāvu",
			presetsRemove: "Dzēst sastāvu",
			presetsNewName: "Norādiet nosaukumu kaujas sastāvam lai to saglabātu",
		...
		"polish": {
			...
			air: "Lotnictwo",
			line1: "Linia frontu",
			line2: "Linia walki na odległość",
			flankLeft: "Lewa flanka",
			flankRight: "Prawa flanka",
			submarine: "Okręty podwodne",
			reserve: "Rezerwa",
			phalanx: "Hoplita",
			steamgiant: "Gigant",
			spearman: "Włócznik",
			...
			cook: "Kucharz",
			medic: "Medyk",
			ship_ram: "Okręt z taranem",
			ship_flamethrower: "Okręt z miotaczem ognia",
			ship_steamboat: "Okręt parowy z taranem",
			ship_ballista: "Balista",
			ship_catapult: "Okręt z katapultą",
			ship_mortar: "Okręt z moździerzem",
			ship_submarine: "Okręt podwodny",
			addSlot: "Wypełnij jeden slot",
			removeSlot: "Opróżnij jeden slot",
			fillRow: "Wypełnij całą linię",
			selectUnits: "Wybierz jednostki",
			assignNone: "Nic",
			assignAll: "Wszystko",
			assignField: "Wypełnij pole bitwy",
			presetsTitle: "Ustawienia",
			presetsAdd: "Dodaj ustawienie",
			presetsRemove: "Usuń ustawienie",
			presetsNewName: "Podaj nazwę ustawienia",
			optShowTitles: "Show battlefield row titles"
		},
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		"portuguese": {
			...
			catapult: "Catapulta",
			mortar: "Morteiro",
			gyrocopter: "Giro-cóptero",
			bombardier: "Balão-Bombardeiro",
			cook: "Cozinheiro",
			medic: "Médico",
			ship_ram: "Trireme",
			ship_flamethrower: "Lança-Chamas",
		...
		"russian": {
			...
			air: "Авиация",
			line1: "Линия фронта",
			line2: "Стрелковые подразделения",
			flankLeft: "Левый фланг",
			flankRight: "Правый фланг",
			...
			cook: "Повар",
			medic: "Доктор",
			ship_ram: "Корабль с тараном",
			ship_flamethrower: "Огнемётный корабль",
			ship_steamboat: "Пароход с тараном",
			ship_ballista: "Корабль с баллистой",
			ship_catapult: "Корабль с катапультой",
			ship_mortar: "Корабль с мортирой",
			ship_submarine: "Подводная лодка",
			addSlot: "Добавить войска в одну клетку",
			removeSlot: "Убрать войска из одной клетки",
			fillRow: "Заполнить войсками один ряд",
			selectUnits: "Выбрать войска",
			assignNone: "Ничего",
			assignAll: "Все",
			assignField: "Заполнить поле боя",
			presetsTitle: "Закладки",
			presetsAdd: "Добавить новую закладку",
			presetsRemove: "Удалить закладку",
			presetsNewName: "Укажите имя новой закладки для сохранения",
		...
		"spanish": {
			...
			air: "Aire",
			line1: "Línea de combate principal",
			line2: "Línea de combate a larga distancia",
		...
		"swedish": {
			...
			line1: "Frontlinje",
			line2: "Distanslinje",
			flankLeft: "Vänster flank",
			flankRight: "Höger flank",
			submarine: "Ubåt",
			reserve: "Reserv",
			phalanx: "Hoplit",
			steamgiant: "Ång-jätte",
		...
		"turkish": {
			garrison: "Garnizon",
			advisor: "Danışman",
			cityLevel: "Şehir Boyutu",
			support: "Destek",
			artillery: "Topçu",
			air: "Hava",
			line1: "Ön Cephe",
			line2: "Uzun Menzilli Savaş Hattı",
			flankLeft: "Sol Kanat",
			flankRight: "Sağ Kanat",
			submarine: "Submarine",
			reserve: "Rezerv",
			phalanx: "Phalanx",
